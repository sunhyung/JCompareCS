...
{
    public struct CompareElement
    {
        public int srcStartIndex;//Line number from source file
        public int dstStartIndex;//Line number from destination file
        public int lineCountOfSameContext;//Line count which indicates how many lines are same from srcLineNo and dstLineNo.

	    public CompareElement(int sIndex, int dIndex, int lCount)
        {
            srcStartIndex = sIndex;
            dstStartIndex = dIndex;
            lineCountOfSameContext = lCount;
        }
    }

    class CRC32
    {
        private static ulong[] CRC32Table = new ulong[256] {
        ...
        public static ulong GetStringCheckSum(string str)
        {
	        ulong crcRes = 0xFFFFFFFF;
    ...
    public class JCompareCore
    {
        private List<ulong> srcHashList_;
        private List<ulong> dstHashList_;
        private List<CompareElement> compareResult_;

        public JCompareCore()
        {
            srcHashList_ = new List<ulong>();
            dstHashList_ = new List<ulong>();
            compareResult_ = new List<CompareElement>();
        }
        ...
        protected void FindLongestCommonSubsequences(int srcBegin, int srcEnd, int dstBegin, int dstEnd)
        {
            CompareElement bestElement = new CompareElement(-1, -1, 0);
            ...
            while (dstIndex <= dstEnd)
            {
                ...
                {
                    int maxLen = Math.Min(srcEnd - srcIndex + 1, dstEnd - dstIndex + 1);
                    ...
                    {
                        if (srcHashList_[srcIndex + i] != dstHashList_[dstIndex + i])
                        {
                            break;
                        }
                        curLongestMatch++;
            ...
            {
                compareResult_.Add(bestElement);
        ...
        {
            srcHashList_.Add(CRC32.GetStringCheckSum(str));
        }

        public void AddDestinationLine(string str)
        {
            dstHashList_.Add(CRC32.GetStringCheckSum(str));
        }
        ...
        {
            return compareResult_.Count;
        }

        public CompareElement GetResultItem(int index)
        {
            if (index >= 0 || index < compareResult_.Count)
            {
                return compareResult_[index];
            }
            else
            {
                return new CompareElement();
            }
        }
    }
}
